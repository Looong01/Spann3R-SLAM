...
class SpatialMemory:
    ...
    def add_mem(self, feat_k, feat_v, pts_cur=None, img_cur=None):
        ...
        if self.mem_count is None:
            ...
            self.mem_count += 1
            self.mem_count = torch.cat(
                (self.mem_count, torch.zeros_like(feat_k[:, :, :1])), dim=1
            )
            self.mem_attn = torch.cat(
                (self.mem_attn, torch.zeros_like(feat_k[:, :, :1])), dim=1
            )
    ...
    def check_sim(self, feat_k, thresh=0.7):
        ...
        # wm: BS, T, 196, C
        wm = self.mem_k[:, -wmem_size:].reshape(
            self.mem_k.shape[0], -1, self.num_patches, self.mem_k.shape[-1]
        )
    ...
    def memory_read(self, feat, res=True):
        ...
        affinity = torch.einsum(
            ...
            self.norm_q(feat),
            self.norm_k(
                self.mem_k.reshape(self.mem_k.shape[0], -1, self.mem_k.shape[-1])
            ),
        )
        ...
        out = torch.einsum(
            ...
            attn,
            self.norm_v(
                self.mem_v.reshape(self.mem_v.shape[0], -1, self.mem_v.shape[-1])
            ),
        )
...
class Spann3R(nn.Module):
    def __init__(
        ...
        # DUSt3R
        self.dust3r = AsymmetricCroCo3DStereo.from_pretrained(
            dus3r_name, landscape_only=True
        )

        # Memory encoder
        self.set_memory_encoder(
            enc_embed_dim=768 if use_feat else 1024, memory_dropout=memory_dropout
        )
        self.set_attn_head()

    def set_memory_encoder(
        self,
        enc_depth=6,
        enc_embed_dim=1024,
        out_dim=1024,
        ...
        self.value_encoder = nn.ModuleList(
            [
                Block(
                    ...
                    rope=self.dust3r.rope if self.mem_pos_enc else None,
                )
    ...
    def encode_image_pairs(self, view1, view2):
        ...
        out, pos, _ = self.dust3r._encode_image(
            torch.cat((img1, img2), dim=0), torch.cat((shape1, shape2), dim=0)
        )
    ...
    def encode_frames(self, view1, view2, feat1, feat2, pos1, pos2, shape1, shape2):
        if feat1 is None:
            feat1, feat2, pos1, pos2, shape1, shape2 = self.encode_image_pairs(
                view1, view2
            )
    ...
    def encode_cur_value(self, res1, dec1, pos1, shape1):
        if self.use_feat:
            ...
            out, pos_v = self.pos_patch_embed(
                res1["pts3d"].permute(0, 3, 1, 2), true_shape=shape1
            )
    ...
    def downstream_head(self, dec, true_shape, num=1):
        with torch.amp.autocast("cuda", enabled=False):
            res = self.dust3r._downstream_head(
                num, [tok.float() for tok in dec], true_shape
            )
    ...
    def offline_reconstruction(self, frames, graph):
        ...
        sp_mem = SpatialMemory(
            self.norm_q, self.norm_k, self.norm_v, mem_dropout=self.mem_dropout
        )
        ...
        while True:
            if feat_k2 is not None:
                ...
                id_n, dec1, dec2, res1, res2, feat2, pos2, shape2, best_conf = (
                    self.find_next_best_view(frames, idx_todo, feat_fuse, pos2, shape2)
                )
